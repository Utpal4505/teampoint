...
export const addProjectMemberService = async (input) => {
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (!workspaceMember || workspaceMember.status === 'REMOVED') {
            throw new ApiError(400, 'User is not an active member of the workspace. Add them to workspace first.');
        }
...
export const updateProjectMemberService = async (input) => {
...
export const removeProjectMemberService = async (projectId, userId) => {
    return prisma.$transaction(async (tx) => {
        const member = await tx.project_Members.findUnique({
            where: {
                projectId_userId: {
...
export const selfExitProjectService = async (projectId, userId) => {
    return prisma.$transaction(async (tx) => {
        const project = await tx.project.findUnique({
            where: { id: projectId },
            select: { id: true, status: true },
        ...
        if (member.role === WorkspaceRole.OWNER) {
            ...
            if (ownerCount === 1) {
                throw new ApiError(400, 'You are the only owner of this project. Transfer ownership to another member before leaving.');
            }
        }
        const removedMember = await tx.project_Members.update({
            where: {
...
export const isProjectMemberService = async (projectId, userId) => {
...
export const getProjectMemberRoleService = async (projectId, userId) => {
    const member = await prisma.project_Members.findUnique({
        where: {
            projectId_userId: {
                projectId: projectId,
...
export const canManageProjectMembersService = async (projectId, userId) => {
    const member = await prisma.project_Members.findUnique({
        where: {
            projectId_userId: {
                projectId: projectId,
...
//# sourceMappingURL=projectMember.service.js.map