...
export const getMeetingService = async (meetingId, userId) => {
...
export const manageParticipantsService = async (input, userId) => {
...
export const completeMeetingService = async (input, userId) => {
...
const assertCanManageMeeting = async (projectId, createdBy, userId, tx) => {
    if (createdBy === userId)
        return;
    ...
    if (!member || !['OWNER', 'ADMIN'].includes(member.role)) {
        throw new ApiError(403, 'Only the meeting creator or project admin can perform this action');
    }
};
