    ...
    return await prisma.$transaction(async (tx) => {
        const project = await tx.project.create({
            data: {
                ...
                description: 'This is a sample project to help you explore how teams collaborate in TeamPoint. Build and launch a simple product in 7 days, focusing only on what matters.',
                workspaceId,
                createdBy: userId,
            },
        });
        ...
        const tasks = await Promise.all([
            tx.tasks.create({
                data: {
                    ...
                    description: 'A simple tool to help small teams manage tasks and discussions in one place.',
                    status: 'DONE',
                    projectId: project.id,
                    createdBy: userId,
                    assignedTo: userId,
        ...
        await tx.message.create({
            data: {
                ...
                createdBy: riya.id,
                content: 'I went through the wireframes — the dashboard layout feels a bit cluttered. Can we simplify the sidebar?',
            },
        });
        ...
        await tx.message.create({
            data: {
                discussionId: designDiscussion.id,
                createdBy: userId,
                content: "Good call. Let's go minimal — one level deep only, no nested menus for v1.",
        ...
        await tx.message.create({
            data: {
                discussionId: designDiscussion.id,
                createdBy: userId,
                content: 'Not for v1. Ship light mode first, dark mode can be a quick win post-launch.',
            },
        });
        ...
        const buildMsg1 = await tx.message.create({
            data: {
                ...
                createdBy: alex.id,
                content: 'I think tasks + discussions are the non-negotiables. Everything else is nice-to-have.',
            },
        });
        await tx.message.create({
            data: {
        ...
        await tx.message.create({
            data: {
                discussionId: buildDiscussion.id,
                createdBy: riya.id,
                content: 'What about file uploads? Users will definitely want to attach things to tasks.',
            },
        });
        await tx.message.create({
            data: {
                discussionId: buildDiscussion.id,
                createdBy: alex.id,
                content: "Maybe just links for now? Full upload support adds infra complexity we don't need yet.",
            },
        });
        await tx.message.create({
            data: {
                discussionId: buildDiscussion.id,
                createdBy: userId,
                content: "Agreed — links only for v1. We'll revisit uploads once we have real user feedback.",
            },
        });
