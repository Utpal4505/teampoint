...
const buildInviteLink = (token, tokenId) => `${env.CLIENT_URL}/invite/${tokenId}/${token}`;
export const sendInviteService = async (input) => {
...
export const getSingleInviteService = async (input) => {
...
export const revokeInviteService = async (input) => {
    const { workspaceId, inviteId } = input;
    const invite = await prisma.invite_Member.findUnique({
        where: { id: inviteId },
...
export const validateInviteService = async (input) => {
...
export const acceptInviteService = async (input) => {
...
//# sourceMappingURL=inviteMember.service.js.map