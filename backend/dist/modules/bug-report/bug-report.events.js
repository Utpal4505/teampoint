...
export const registerBugReportEvents = () => {
    eventBus.on('BUG_REPORT_CREATED', async (bugReportId, userId) => {
        await processBugReport(bugReportId, userId);
    });
    eventBus.on('BUG_AI_PROCESSED', async (bugReportId, userId) => {
        await processBugAIReport(bugReportId, userId);
    });
    eventBus.on('BUG_REPORT_DUPLICATE', async (bugReportId) => {
...
//# sourceMappingURL=bug-report.events.js.map