...
const normalizeApiRoute = (apiRoute) => {
    if (!apiRoute || apiRoute.trim().toLowerCase() === 'n/a')
        return 'Not provided';
    return apiRoute;
};
const extractMetaSignals = (metadata) => {
    if (!metadata)
        return 'Not provided';
    const signals = [];
    if (metadata.browser)
    ...
    if (metadata.os)
        signals.push(`OS: ${metadata.os} ${metadata.os_version ?? ''}`);
    if (metadata.device_type)
        signals.push(`Device: ${metadata.device_type}`);
    if (metadata.viewport)
        signals.push(`Viewport: ${metadata.viewport}`);
    if (metadata.network_status)
        signals.push(`Network: ${metadata.network_status}`);
    if (metadata.pageLoadTimeMs) {
        const seconds = Math.round(metadata.pageLoadTimeMs / 1000);
...
`;
export const buildBugUserPrompt = (bug) => {
    const payload = {
        title: bug.title,
        ...
        metaSignals: extractMetaSignals(bug.metadata),
    };
