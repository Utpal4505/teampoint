...
export const archiveWorkspaceService = async ({ workspaceId, }) => {
    return handlePrismaNotFound(prisma.workspace.update({
        where: {
            id: workspaceId,
            status: 'ACTIVE',
        },
...
export const updateWorkspaceMemberRoleService = async ({ workspaceId, actorId, targetUserId, role, }) => {
    return prisma.$transaction(async (tx) => {
        const member = await tx.workspace_Members.findUnique({
            where: {
                workspaceId_userId: {
